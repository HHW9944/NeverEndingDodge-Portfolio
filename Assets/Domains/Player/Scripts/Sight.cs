...
public class Sight : MonoBehaviour
{
    ...
    void DetectTargets()
    {
        ...
        foreach (Collider target in targetsInViewRadius)
        {
            ...
            float angleToTarget = Vector3.Angle(detectionDirection.normalized, directionToTarget);
    ...
    private void OnDrawGizmos()
    {
        ...
        // 시야각 표시 (오른쪽 방향)
        Vector3 rightDirection = Quaternion.Euler(0, viewAngle / 2, 0) * detectionDirection;
        Gizmos.color = Color.green;
        Gizmos.DrawRay(transform.position, rightDirection * detectionRadius);

        // 시야각 표시 (왼쪽 방향)
        Vector3 leftDirection = Quaternion.Euler(0, -viewAngle / 2, 0) * detectionDirection;
        Gizmos.color = Color.green;

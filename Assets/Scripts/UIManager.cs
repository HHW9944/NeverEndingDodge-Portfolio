...
public class UIManager : MonoBehaviour
{
    ...
    {
        playTimeResult.text = "Play Time :  " + (GameManager.PLAYTIME - GameManager.timer).ToString("F1");
    ...
    private void StartVignetteEffect()
    {
        if (vignette != null)
        {
            ...
            vignette.intensity.Override(0.8f); // Vignette ???? ???? (??竅 ???? ????)
        }
    }
